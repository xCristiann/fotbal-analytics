...
function computeH2HStats(h2hFixturesFiltered: any[]): HeadToHeadStats | undefined {
  ...
  return { matchesCount: h2hFixturesFiltered.length, bttsRate: bttsCount / h2hFixturesFiltered.length, over25Rate: over25Count / h2hFixturesFiltered.length };
}
...
export async function GET(request: Request) {
  ...
  // Vedem ce meciuri au DEJA predictii salvate, ca sa nu le refacem
  // degeaba - fiecare rulare trebuie sa avanseze la meciuri noi, nu
  // sa repete aceleasi primele N din lista.
  const eligibleForAnalysis = candidateFixtures.filter((c) => fullAnalysisDates.has(c.fixtureDateStr));
  let alreadyAnalyzedIds = new Set<string>();
  if (eligibleForAnalysis.length > 0) {
    const idsToCheck = eligibleForAnalysis.map((c) => c.matchRow.id);
